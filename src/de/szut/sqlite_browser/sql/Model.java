...
public class Model {
	...
	public void executeQuery(String query, boolean fromTextfield) {
		...
		try {
			/***
			 * Check if Query is from Textfield or JTree
			 */
			if(fromTextfield){
				query1 = query;
			 ...
			 */
			if(panel.getLimitStatus() && isNumeric(panel.getLimitFrom(), panel.getLimitTo())){
				build = new StringBuilder();
				build.append(query1);
				build.append(" Limit ");
				if (panel.getLimitFrom().equals("") && panel.getLimitTo().equals("")){
					build = new StringBuilder();
					build.append(query1);
			...
			panel.updateDataList(data, columnNames);
			}
	...
	/***
	 * Beendet die Connection setzt die Status anzeige zurück sowiet restet die Tabel und den Baum
	 */
	...
	{  
	  try  {  
	    ...
	    int to = Integer.parseInt(limitTo);
	    if (from < 0) {
	    	return false;
	    }
	  }  
